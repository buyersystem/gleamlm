...
m = GleamLMModel(vocab_size=12002, d_model=768, num_layers=12, num_heads=12, num_kv_heads=6,
                  d_ff=2048, dropout=0.0, max_seq_len=2048, use_flash_attn=True)
total = sum(p.numel() for p in m.parameters())
embed = sum(p.numel() for n, p in m.named_parameters() if 'token_embed' in n)
ffn_params = sum(p.numel() for n, p in m.named_parameters() if 'W_gate' in n or 'W_up' in n or 'W_down' in n)
