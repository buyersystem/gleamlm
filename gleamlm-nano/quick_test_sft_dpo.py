...
model = GleamLMModel(vocab_size=VOCAB, d_model=512, num_layers=12,
                      num_heads=8, num_kv_heads=4, d_ff=1365, max_seq_len=512).to(device)
...
sft_model = GleamLMModel(vocab_size=VOCAB, d_model=512, num_layers=12,
                          num_heads=8, num_kv_heads=4, d_ff=1365, max_seq_len=512, tie_weights=True).to(device)
...
policy = GleamLMModel(vocab_size=VOCAB, d_model=512, num_layers=12,
                       num_heads=8, num_kv_heads=4, d_ff=1365, max_seq_len=512).to(device)
ckpt_sft = torch.load("checkpoints/sft_test/sft_best.pt", map_location=device)
policy.load_state_dict(ckpt_sft["model_state_dict"])

# Reference model (冻结)
ref = GleamLMModel(vocab_size=VOCAB, d_model=512, num_layers=12,
                    num_heads=8, num_kv_heads=4, d_ff=1365, max_seq_len=512).to(device)
