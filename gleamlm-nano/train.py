...
def train_one_epoch(model, train_loader, optimizer, scheduler, criterion, raw_criterion,
    ...
    for batch_idx, (input_ids, target_ids) in enumerate(pbar):
        ...
        autocast_ctx = torch.amp.autocast(_amp_device) if hasattr(torch.amp, 'autocast') else torch.cuda.amp.autocast()
        with autocast_ctx:
...
def main():
    ...
    if '--config' not in sys.argv:
        _default_config = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'configs', 'nano.yaml')
