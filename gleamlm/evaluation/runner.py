...
def main():
    ...
    parser.add_argument("--benchmarks", type=str, default="ppl,knowledge",
                        help="逗号分隔：ppl,knowledge,ceval,cmmlu")
    parser.add_argument("--data_dir", type=str, required=True,
                        help="数据目录（如 data/lite_data）")
    ...
    # 加载模型和分词器
    print("=" * 60)
    print(f"GleamLM Evaluation Runner")
    ...
    for name, data in all_results.items():
        if "ppl" in data:
            ...
            print(f"  {name:>10}: Acc={data['accuracy']:.2%}, {data.get('correct', '?')}/{data.get('total', '?')}")
