...
from gleamlm.utils.torch_utils import safe_autocast, get_lr_cosine
...
def train_one_epoch(model, train_loader, optimizer, scheduler, criterion,
    ...
    for batch_idx, (input_ids, target_ids) in enumerate(pbar):
        ...
        sync_context = model.no_sync() if (not is_accum_step and args.world_size > 1) else nullcontext()
        with sync_context:
...
def main():
    ...
    parser.add_argument("--data_dir", type=str,
                        default=os.path.join(_root_dir, 'data', 'lite_data'))
    ...
    parser.add_argument("--config", type=str, default=os.path.join(_root_dir, 'configs', 'lite.yaml'),
                        help="YAML 配置文件路径")
    ...
    parser.add_argument("--ids_prefix", type=str, default="",
                        help="预分词文件前缀，用于区分不同分词器")
    ...
    if config_args.config:
        ...
        defaults = {a.dest: parser.get_default(a.dest)
                    for a in parser._actions if a.dest != 'help' and a.dest != 'config'}
    ...
    if args.local_rank == 0:
        ...
        print("=" * 60)
        print(f"  模型: d={args.d_model}, L={args.num_layers}, heads={args.num_heads}/{args.num_kv_heads}")
        print(f"  词表: {args.vocab_size}, seq={args.max_seq_len}, dropout={args.dropout}")
        print(f"  批次: {args.batch_size} x accum {args.accumulate_grad} = effective {args.batch_size * args.accumulate_grad}")
        print(f"  学习率: {args.lr:.0e}, Cosine (warmup={args.warmup_ratio}, min_lr_ratio={args.min_lr_ratio})")
    ...
    train_dataset = LMDataset(args.data_dir, tokenizer, args.max_seq_len, "train",
                               max_chars=args.max_train_chars, ids_prefix=args.ids_prefix)
