...
sys.path.insert(0, os.path.dirname(_SCRIPT_DIR))  # 确保 gleamlm 可导入
...
# Model Tests (Lite 87M)
print("=" * 50)
print("Model Tests (GleamLM-Lite 87M)")
print("=" * 50)
...
def test_parameter_count():
    ...
    model = GleamLMModel(
        vocab_size=VOCAB_SIZE, d_model=D_MODEL, num_layers=12,
    ...
    assert 80_000_000 < total < 95_000_000, f"Unexpected param count: {total / 1e6:.1f}M"
    return model
...
def test_long_sequence():
    with torch.no_grad():
        ...
        logits, _ = model(input_ids)
    assert logits.shape == (2, 256, VOCAB_SIZE), f"Expected (2,256,{VOCAB_SIZE}) got {logits.shape}"
...
def test_flash_attn():
    """验证 Flash Attention (sdpa) 模式能正常 forward"""
...
def test_encode_decode_english():
    text = "Hello, World!"
    ids = tokenizer.encode(text, add_bos=False, add_eos=False)
    decoded = tokenizer.decode(ids)
    assert decoded == text, f"Round-trip failed: '{text}' -> '{decoded}'"
...
test("test_encode_decode_english", test_encode_decode_english)
...
def test_sampler_shape():
    ...
    assert 0 <= token.item() < VOCAB_SIZE, f"Token out of range: {token.item()}"
...
def test_dataset_creation():
    ...
    data_dir = os.path.join(os.path.dirname(_SCRIPT_DIR), "..", "data", "lite_data")
    if not os.path.exists(os.path.join(data_dir, "valid.txt")):
        print("    SKIP: data/lite_data/valid.txt not found")
        return
    ds = LMDataset(data_dir, tokenizer, 2048, "valid", max_chars=500_000, augment=False)
    print(f"    Samples: {len(ds)}")
    assert len(ds) > 0, "Dataset should have at least 1 sample"


test("test_dataset_creation", test_dataset_creation)
...
test("test_collate_fn", test_collate_fn)
