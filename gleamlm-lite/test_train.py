...
# 构建 Lite 87M 模型
model = GleamLMModel(
    vocab_size=12002, d_model=768, num_layers=12,
...
optimizer = torch.optim.AdamW(model.parameters(), lr=4e-4, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.01)

total_steps = math.ceil(len(loader) / 2) * 2
scheduler = torch.optim.lr_scheduler.LambdaLR(
    optimizer,
    lambda s: get_lr_cosine(s, total_steps, warmup_ratio=0.01, min_lr_ratio=0.1)
)
scaler = torch.amp.GradScaler('cuda')

print(f"\n训练 {total_steps} 步 (batch=2, accum=2, epochs=2, seq=256)...")
model.train()
global_step = 0
optimizer.zero_grad()
